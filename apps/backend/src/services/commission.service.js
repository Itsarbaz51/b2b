...
export class CommissionSettingService {
  ...
  static async checkUserPricingRule(userId, serviceProviderMappingId) {
    ...
    if (!rule) {
      throw ApiError.badRequest(
        "Pricing rule/commission is not configured for this service. Please contact your administrator."
      );
    }
...
export class CommissionPaymentMethodService {
  static async upsert(payload) {
    ...
    if (!commission) throw ApiError.notFound("Commission setting not found");

    const serviceCode = commission.serviceProviderMapping?.service?.code;
